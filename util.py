...
def dict_from_tar_list(filename):
    ...
    # Create dictionary
    target_set = {headers[i]: values[i] for i in range(column_number-1)}
...
def check_nans(target_dict):
    ...
    by_column = [list(np.where(target_dict[key] != 0.)[0]) for key in dict_keys]
...
def plotable_hz_bounds(temp = np.linspace(2600, 7200, 5000),
    ...
    bounds = ["oi", "ci", "co", "oo"]
    hz_bounds = {key: habitable_zone_distance(temp, lbol, key) for key in bounds}
...
def habitable_zone_distance(T_eff, L, est_ident):
	...
	:param L: NDARRAY,
		Stellar bolometric luminosity in solar units
	:param est_ident: STR,
